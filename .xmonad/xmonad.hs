...
-- myTerminal           = "sakura"   -- which terminal software to use
...
-- preset keybindings.
myLauncher = "$(yeganesh -x -- -fn '-*-terminus-*-r-normal-*-*-120-*-*-*-*-iso8859-*' -nb '#000000' -nf '#727272' -sb '#eeeeee' -sf '#333333')"
...
devLayout = smartBorders(avoidStruts(
  -- ResizableTall layout has a large master window on the left,
  -- and remaining windows tile on the right. By default each area
  -- takes up half the screen, but you can resize using "super-h" and
  -- "super-l".
...
--    , ((myModMask, xK_p), spawn "synapse")
    , ((myModMask, xK_p), spawn "synapse")
    , ((myModMask .|. mod2Mask, xK_space), spawn "synapse")
--    , ((myModMask .|. mod1Mask, xK_space), spawn myLauncher)
    , ((myModMask, xK_u), focusUrgent)
    , ((0, 0x1008FF12), spawn "amixer -q set Master toggle")
    , ((0, 0x1008FF11), spawn "amixer -q set Master 10%-")
    , ((0, 0x1008FF2C), spawn "amixer set Capture toggle")
    , ((myModMask              , xK_q     ), kill)
  ]
...
myManagementHooks = [
  ...
  , (className =? "jetbrains-studio") --> doF (W.shift "2:Dev")
  ...
  , (className =? "Rhythmbox") --> doF (W.shift "7:Media")
  , (className =? "Gimp-2.8") --> doF (W.shift "9:Hub")
  ]
